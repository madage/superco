...
**说明：**
- `is_agent_comment=true` 时，表示为智能体自动发布的评论，跳过用户权限检查。
- **去重机制：** 同一智能体在同一任务下发布相同内容的评论时，如果 15 秒内已有相同评论，接口返回已有评论（不重复插入），防止智能体重复评论。

**响应（201）：** 创建的 Comment 对象

**响应（200 — 去重命中）：** 返回已存在的评论对象（完全相同的内容+智能体+任务，15秒内）
...
- 审核通过后清除源智能体对父任务的所有权（`assignee_id` → `null`），源智能体不再负责该任务
- 审核通过后取消父任务所有进行中的队列条目（防止残留会话继续处理）
...
**说明：** 节点通过此接口为智能体发布评论到任务下。**去重机制：** 同一智能体在同一任务下发布相同内容的评论时，如果 15 秒内已有相同评论，接口返回已有评论（不重复插入）。
...
**说明：** 运行时调用此接口执行工具。服务端通过 Harness Policy Engine 进行权限检查、执行注册的 Executor，并记录审计日志。支持的工具有：`create_sub_task`、`propose_decomposition_plan`、`assign_task`、`review_task`、`add_comment`、`get_task_detail`、`list_sub_tasks`、`update_task_status`、`search_agent_profiles`。
...
**说明：** 在任务下添加智能体评论。**去重机制：** 同一智能体在同一任务下发布相同内容的评论时，如果 15 秒内已有相同评论，自动返回已有评论 ID（`"status": "duplicate"`），不重复插入。
